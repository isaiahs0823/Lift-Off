  ...
  if (!session) {
    return (
      <div className="space-y-4">
        ...
        <div className="text-sm text-v5-subtext">Session not found.</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <div className="text-[11px] uppercase tracking-widest text-v5-red">{recap.planName}</div>
          <div className="text-xl font-bold text-white mt-0.5">Session recap</div>
          <div className="text-sm text-v5-subtext mt-0.5">{formatSessionDuration(recap.durationSec)}</div>
        </div>
        {onBack && (
          <button onClick={onBack} className="shrink-0 text-xs uppercase tracking-widest text-v5-subtext hover:text-v5-red">
            ← Back
          </button>
        )}
      </div>
      ...
      {recap.alternateGym && (
        <div className="border border-white/10 bg-v5-elevated p-3 space-y-1">
          ...
          {recap.alternateGym.differentEquipmentCount > 0 ? (
            ...
            <div className="text-xs text-v5-subtext">No equipment differences flagged this session.</div>
          )}
        </div>
      )}
        ...
        <div className="border border-v5-red/25 bg-v5-elevated p-4 space-y-2.5">
          ...
          {recap.prs.map((pr, i) => (
            <div key={i} className="space-y-0.5">
              ...
              <div className="flex items-center gap-2 text-xs text-v5-subtext">
                <span>
        ...
        <div className="border border-white/10 bg-v5-elevated p-4 space-y-3">
          ...
          {recap.declines
            ...
            .map((e) => (
              <div key={e.exId} className="space-y-0.5">
                <div className="text-sm font-bold text-white">{e.name}</div>
                <div className="text-xs text-v5-subtext">{e.progression.message}</div>
              </div>
            ))}
        </div>
      )}
      ...
      <div className="space-y-3">
        ...
        {recap.perExercise
          .filter((e) => e.nextTime)
          .map((e) => (
            <div key={e.exId} className="border-b border-white/[0.06] pb-2.5 last:border-b-0">
              <div className="text-sm font-bold text-white">{e.name}</div>
      ...
      {onViewFullSession && (
        <button onClick={onViewFullSession} className="w-full text-center text-[11px] uppercase tracking-widest text-v5-subtext hover:text-v5-red py-1">
          View full session
        </button>
      )}
    </div>
