  ...
  if ((status === "denied" || status === "unsupported" || status === "error") && step === "scanning") {
    return (
      <div className="space-y-6">
        ...
        <p className="text-sm text-neutral-400">BRK needs camera access to scan food. {errorMessage}</p>
        ...
        <div className="border border-neutral-800 bg-charcoal-panel p-4 space-y-3">
          <label className="block text-[11px] uppercase tracking-widest text-neutral-500">Enter barcode manually</label>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-charcoal-deep border border-neutral-800 text-neutral-100 px-3 py-2 text-sm focus:outline-none focus:border-red-700"
            />
            <button onClick={submitManualBarcode} className="shrink-0 px-4 py-2 text-xs uppercase tracking-widest font-bold border bg-red-700 border-red-700 text-white hover:bg-red-600">
              Look Up
            </button>
          </div>
        </div>
        <button onClick={onManualEntry} className="w-full py-3 text-xs uppercase tracking-widest font-bold border bg-red-700 border-red-700 text-white hover:bg-red-600">
          Enter Food Manually
        </button>
      </div>
    );
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="border border-red-900/40 bg-charcoal-panel p-4 space-y-3">
          <div>
            {product.brand && <div className="text-xs text-neutral-500">{product.brand}</div>}
            <div className="text-lg font-bold text-white">{product.name}</div>
            <div className="text-[11px] text-neutral-600 mt-1">Barcode: {product.barcode}</div>
          </div>
            ...
            <>
              <div className="grid grid-cols-4 gap-2 text-center">
                <div>
                  ...
                  <div className="text-[10px] uppercase tracking-widest text-neutral-500">Calories</div>
                </div>
                <div>
              ...
              <div className="flex gap-2">
                <button onClick={logProduct} className="flex-1 py-2.5 text-xs uppercase tracking-widest font-bold border bg-red-700 border-red-700 text-white hover:bg-red-600">
                  Add Food
                </button>
                <button onClick={() => setEditingServing(true)} className="flex-1 py-2.5 text-xs uppercase tracking-widest font-bold border border-neutral-800 text-neutral-300 hover:border-neutral-600">
                  Edit Serving
                </button>
              </div>
            </>
  ...
  if (step === "notFound") {
    return (
      <div className="space-y-6">
        ...
        <div className="border border-amber-900/40 bg-charcoal-panel p-4 space-y-2">
          <div className="text-sm text-neutral-300">Barcode: {barcode}</div>
          <div className="text-sm text-amber-500">We don't have this product yet.</div>
        </div>
        <button onClick={() => onScanLabelForBarcode(barcode)} className="w-full py-3 text-xs uppercase tracking-widest font-bold border bg-red-700 border-red-700 text-white hover:bg-red-600">
          Scan Nutrition Label
        </button>
        <button onClick={onManualEntry} className="w-full py-3 text-xs uppercase tracking-widest font-bold border border-neutral-800 text-neutral-300 hover:border-neutral-600">
          Enter Manually
        </button>
      </div>
    );
  ...
  if (step === "lookupFailed") {
    return (
      <div className="space-y-6">
        ...
        <p className="text-sm text-neutral-400">We scanned barcode {barcode}, but couldn't reach the product database. Check your connection and try again.</p>
        <button onClick={() => runLookup(barcode)} className="w-full py-3 text-xs uppercase tracking-widest font-bold border bg-red-700 border-red-700 text-white hover:bg-red-600">
          Try Again
        </button>
        <button onClick={() => onScanLabelForBarcode(barcode)} className="w-full py-3 text-xs uppercase tracking-widest font-bold border border-neutral-800 text-neutral-300 hover:border-neutral-600">
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="text-xs uppercase tracking-widest text-neutral-500 hover:text-red-500"
        >
          Cancel
        </button>
      </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="w-[80%] aspect-[8/5] border-2 border-red-600">
            <div className="w-full h-full flex items-center justify-center">
              <span className="text-[11px] uppercase tracking-widest text-red-500 bg-black/60 px-2 py-1">Position Barcode Inside Frame</span>
            </div>
          </div>
        </div>
        {status === "requesting" && (
      ...
      <p className="text-xs text-neutral-500 text-center">Hold your product's barcode steady inside the frame.</p>

      {showManualBarcode ? (
        <div className="border border-neutral-800 bg-charcoal-panel p-4 space-y-3">
          <label className="block text-[11px] uppercase tracking-widest text-neutral-500">Type the barcode number</label>
          <div className="flex gap-2">
            <input
              type="text"
              inputMode="numeric"
        ...
        <button onClick={() => setShowManualBarcode(true)} className="w-full py-2.5 text-xs uppercase tracking-widest text-neutral-500 hover:text-red-500 border border-neutral-800">
          Type Barcode Instead
        </button>
      )}
      <button onClick={onManualEntry} className="w-full py-2.5 text-xs uppercase tracking-widest text-neutral-500 hover:text-red-500 border border-neutral-800">
        Enter Food Manually
      </button>
    </div>
  );
